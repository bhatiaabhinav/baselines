    ...
    def __init__(self, session: tf.Session, name, ob_shape, ac_shape, gamma=0.99, ob_dtype='float32', delta_R_lr=1e-3, a_lr=1e-3, n_lr=1e-3, v_lr=1e-3, goals_lr=1e-3, use_layer_norm=True, tau=0.001):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('n'):
                ...
                goals_feed = tf.case([(self.connect_graphs, lambda:self.goals)], default=lambda:self.n_goals_feed)
                feed = tf.concat([self.states_feed, goals_feed], axis=-1)
                h1 = fc(feed, 'h1', 128)
                h2 = fc(h1, 'h2', 64)
                self.n = fc(h2, 'n', 1, act=lambda x:x)[:, 0]
            with tf.variable_scope('delta_R'):
                self.delta_R_goals_feed = tf.placeholder(dtype=ob_dtype, shape=[None] + list(ob_shape))
                goals_feed = tf.case([(self.connect_graphs, lambda:self.goals)], default=lambda:self.delta_R_goals_feed)
                feed = tf.concat([self.states_feed, goals_feed], axis=-1)
                h1 = fc(feed, 'h1', 128)
                h2 = fc(h1, 'h2', 64)
            ...
            with tf.variable_scope('a'):
                ...
                goals_feed = tf.case([(self.connect_graphs, lambda:self.goals)], default=lambda:self.a_goals_feed)
                feed = tf.concat([self.states_feed, goals_feed], axis=-1)
                h1 = fc(feed, 'h1', 128)
                h2 = fc(h1, 'h2', 64)
            ...
            with tf.variable_scope('v'):
                ...
                states_feed = tf.case([(self.connect_graphs, lambda:self.goals)], default=lambda:self.v_states_feed)
        ...
        self.delta_R_training_error = tf.reduce_mean(tf.square(self.delta_R - self.delta_R_training_delta_R_feed)/2)
        optimizer = tf.train.AdamOptimizer(learning_rate=delta_R_lr)
        delta_R_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='{0}/delta_R'.format(name))
        with tf.control_dependencies(update_ops):
            self.delta_R_training_op = optimizer.minimize(self.delta_R_training_error, var_list=delta_R_vars)
        ...
        goals_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='{0}/goals'.format(name))
        with tf.control_dependencies(update_ops):
