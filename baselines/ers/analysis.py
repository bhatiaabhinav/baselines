...
# def analyse_q(sys_args_dict, sess: tf.Session, env_id, wrappers, seed=0, test_env_seed=42, actor=None, load_path=None, **kwargs):
#     np.random.seed(seed)
...
#         if 'ERS' in env_id:
#             actor_kwargs = dict(
#                 sys_args_dict, log_transform_max_x=env.metadata['nambs'], log_transform_t=env.metadata['log_transform_alloc_t'])
#         else:
#             actor_kwargs = sys_args_dict
#         actor = Actor(sess, 'actor', env.observation_space.shape,
#                       env.action_space.shape, **actor_kwargs)
